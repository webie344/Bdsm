...
// Supported video formats - EXPANDED FOR DOWNLOADED VIDEOS
...
class StreamManager {
    ...
    async createStream(videoData, headline, description, category, isLocalFile = false) {
        try {
            ...
            if (isLocalFile && videoData instanceof File) {
                ...
                // Upload to Cloudinary with enhanced transformation for problematic formats
                const videoUrl = await this.uploadVideoToCloudinary(videoData);
                const thumbnailUrl = await this.generateVideoThumbnail(videoData);
                ...
                // Handle external URL
                if (!this.isValidVideoUrl(videoData)) {
                    throw new Error('Please enter a valid video URL from YouTube, Vimeo, Dailymotion, or direct video link');
                }
                ...
                streamData = {
                    ...
                    isActive: true,
                    sortTimestamp: new Date().getTime(),
                    embedUrl: this.generateEmbedUrl(videoData),
                    thumbnailUrl: this.generateThumbnailUrl(videoData),
                    isPhoneVideo: false,
    ...
    needsConversion(file) {
        ...
        console.log('Video conversion needed:', needsConversion, file.type, file.name);
    ...
    isDownloadedVideo(file) {
        const downloadedIndicators = [
            ...
            file.size < 10000000 && file.type === 'video/mp4', // Small MP4 files often from social media
            file.name.includes('-') && file.name.split('-').length > 3, // Social media naming patterns
        ];
    ...
    async isPortraitVideo(file) {
        return new Promise((resolve) => {
            ...
            video.addEventListener('error', () => {
                URL.revokeObjectURL(video.src);
                resolve(false); // Default to landscape if can't determine
            });
    ...
    // Validate video file for phone compatibility - MORE LENIENT
    async validateVideoFile(file) {
        ...
        // Check if file is a video - MORE LENIENT CHECK
        if (!file.type.startsWith('video/') && !this.isLikelyVideoFile(file)) {
            throw new Error('Please select a valid video file');
        }

        // Check supported formats - WARNING INSTEAD OF ERROR
        ...
        if (!isSupportedFormat && !isSupportedExtension) {
            console.warn('Video format might need conversion:', file.type, file.name);
        ...
        if (this.needsConversion(file)) {
            console.warn('Video may need conversion:', file.type, file.name);
    ...
    isLikelyVideoFile(file) {
        const videoIndicators = [
            ...
            file.size > 100000, // Larger than 100KB
            file.type === '' || file.type === 'application/octet-stream' // Unknown types
        ];
    ...
    isCommonPhoneFormat(file) {
        ...
        return phoneFormats.includes(file.type) || 
               ...
               file.name.toLowerCase().includes('movi') || // Common phone video naming
               file.name.toLowerCase().includes('vid_') ||   // Common phone video naming
               file.name.toLowerCase().includes('camera') || // Camera videos
               file.name.toLowerCase().includes('record');   // Screen recordings
    }
    ...
    isLikelyPhoneVideo(file) {
        return this.isCommonPhoneFormat(file) || 
               ...
               file.type === 'video/quicktime' || // iOS videos
               file.type === 'video/mp4' || // Most phone videos
               file.type === 'video/3gpp'; // Older Android
    }

    // Upload video to Cloudinary - FIXED VERSION
    async uploadVideoToCloudinary(videoFile) {
        ...
        try {
            console.log('Uploading video to Cloudinary with preset:', cloudinaryConfig.uploadPreset);
    ...
    // Generate thumbnail from video file with better error handling
    async generateVideoThumbnail(file) {
        return new Promise((resolve) => {
            ...
            const generateThumb = async () => {
                try {
                    // Try multiple time points for better thumbnails
                    const timePoints = [0.1, 1, 3, 5];
                    let success = false;
                    ...
                    for (let timePoint of timePoints) {
                        try {
                            ...
                            canvas.toBlob(async (blob) => {
                                if (blob) {
                                    try {
                                        const thumbnailUrl = await this.uploadImageToCloudinary(blob);
                            ...
                            console.warn(`Thumbnail generation failed at ${timePoint}s:`, seekError);
                            continue;
                        }
                    }
                    
                    if (!success) {
                        throw new Error('Could not generate thumbnail from video');
                    }
                    
                } catch (error) {
                    console.error('Thumbnail generation error:', error);
                    resolve('images-defaultse-profile.jpg');
                }
            };
    ...
    async uploadImageToCloudinary(imageBlob) {
        const formData = new FormData();
    ...
    isValidVideoUrl(url) {
        try {
            ...
            const patterns = [
                ...
                /^(https?:\/\/)?.*\.(mp4|webm|ogg|mov|m4v|3gp|avi|mkv|wmv|flv|mpg|mpeg)(\?.*)?$/i
            ];
    ...
    generateEmbedUrl(url) {
        try {
            if (url.includes('youtube.com/watch?v=')) {
                ...
                const videoId = url.split('dailymotion.com/')[1]?.split('/').pop();
    ...
    generateThumbnailUrl(url) {
        try {
            if (url.includes('youtube.com/watch?v=') || url.includes('youtu.be/')) {
                ...
                return videoId ? `https://img.youtube.com/vi/${videoId}/maxresdefault.jpg` : 'images-defaultse-profile.jpg';
            } else if (url.includes('vimeo.com/')) {
                const videoId = url.split('vimeo.com/')[1]?.split('/').pop();
                return 'images-defaultse-profile.jpg';
            } else if (url.includes('dailymotion.com/')) {
                const videoId = url.split('dailymotion.com/')[1]?.split('/').pop();
                return videoId ? `https://www.dailymotion.com/thumbnail/video/${videoId}` : 'images-defaultse-profile.jpg';
            }
    ...
    listenToStreams(callback, category = 'all') {
        try {
            ...
            console.error('Error setting up streams listener:', error);
            return () => {};
        }
    }
    ...
    listenToViewerCount(streamId, callback) {
        try {
            ...
            const unsubscribe = onSnapshot(streamRef, (doc) => {
                ...
                console.error('Viewer count listener error:', error);
                callback(0);
            });
            ...
            console.error('Error setting up viewer count listener:', error);
            return () => {};
        }
    }
    ...
    async getTotalViewers() {
        try {
            ...
            console.error('Error getting total viewers:', error);
            return 0;
        }
    }
...
class VideoPlayer {
    ...
    addStyles() {
        if (!document.getElementById('videoPlayerStyles')) {
            ...
            styles.textContent = `
                ...
                /* FIXED: Ensure video controls are always visible */
                ...
                @media (max-width: 768px) {
                    ...
                    /* Ensure mobile video controls are visible */
                    .cloudinary-video-container video {
                        max-height: 60vh;
                    }
                }
    ...
    open(stream) {
        ...
        if (stream.videoType === 'cloudinary') {
            if (stream.isPhoneVideo) {
                ...
                formatText = `🔄 Converted Video • ${stream.videoFileName || 'Downloaded video'}`;
            } else {
                formatText = `📹 Uploaded Video • ${stream.videoFileName || 'Video file'}`;
            }
        } else {
            formatText = '🌐 External Video';
        }
        ...
        } else if (stream.videoType === 'external' && stream.embedUrl) {
            this.loadExternalVideo(stream, contentContainer);
        } else {
        ...
        container.innerHTML = `
            <div class="cloudinary-video-container">
                <div class="video-loading" id="videoLoading">
                    ...
                    Loading video...
                    ${stream.needsConversion ? '<div style="margin-top: 10px; font-size: 12px; color: #ffa500;">Converting video for better compatibility...</div>' : ''}
    ...
    setupVideoPlayer(stream) {
        ...
        videoElement.addEventListener('error', (e) => {
            ...
            if (videoElement.error) {
                switch(videoElement.error.code) {
                    ...
                    case videoElement.error.MEDIA_ERR_DECODE:
                        ...
                        detailsMsg = 'This often happens with videos downloaded from social media. The video is being converted for better compatibility.';
                        break;
            ...
            if (stream.needsConversion) {
                detailsMsg = 'This video was converted for better compatibility. If it still doesn\'t play, try uploading the original video file.';
            }
        ...
        this.loadVideoWithRetry(videoElement, loadingElement, errorElement, stream);
    }
    ...
    loadVideoWithRetry(videoElement, loadingElement, errorElement, stream, retryCount = 0) {
        ...
        const timeoutDuration = stream.needsConversion ? 45000 : 30000; // Longer timeout for converted videos
        
        const loadTimeout = setTimeout(() => {
            if (loadingElement.style.display !== 'none' && videoElement.readyState < 2) {
                console.warn(`Video load timeout (attempt ${retryCount + 1})`);
    ...
    loadExternalVideo(stream, container) {
        container.innerHTML = `
        ...
        placeholder.addEventListener('click', () => {
            this.loadEmbeddedVideo(stream.embedUrl, container);
        });
    }

    loadEmbeddedVideo(embedUrl, container) {
        container.innerHTML = `
            <div class="video-embed-container">
                <iframe src="${embedUrl}" 
    ...
    reloadCurrentVideo() {
        if (this.currentVideoElement) {
            ...
            setTimeout(() => {
                ...
                if (playPromise !== undefined) {
                    playPromise.catch(e => {
                        console.log('Play after reload prevented:', e);
                    });
                }
            }, 100);
        }
...
async function initializeStreamPage() {
    ...
    if (!currentUser && (currentPage === 'poststream' || currentPage === 'stream')) {
        console.log('No user, redirect will happen');
        return;
    }
    ...
    switch(currentPage) {
        ...
        case 'stream':
            ...
            break;
        default:
            console.log('No stream page initialization needed for:', currentPage);
    }
}
...
function initializePostStreamPage() {
    ...
    if (!streamForm || !videoUrlInput || !videoFileInput) {
        console.error('Required form elements not found!');
        return;
    }

    console.log('Initializing post stream page for user:', currentUser?.email);

    // Setup upload method switching
    setupUploadMethodSwitching();
    ...
    videoFileInput.addEventListener('change', function(e) {
        ...
        if (file) {
            ...
            if (needsConversion) {
                statusMessage += ' - This video may need conversion for better compatibility';
    ...
    streamForm.addEventListener('submit', async (e) => {
        ...
        // Get the active upload method
        const activeMethod = document.querySelector('.upload-option.active').dataset.method;
        ...
        // Validation based on active method
        if (activeMethod === 'url' && !videoUrl) {
            showError('Please enter a video URL');
            return;
        }

        if (activeMethod === 'file' && !videoFile) {
        ...
        if (activeMethod === 'url' && videoUrl && !streamManager.isValidVideoUrl(videoUrl)) {
            showError('Please enter a valid video URL from YouTube, Vimeo, Dailymotion, or direct video link');
            videoUrlInput.focus();
            return;
        }
        ...
        try {
            ...
            if (activeMethod === 'file' && videoFile) {
                ...
                streamId = await streamManager.createStream(videoFile, headline, description, category, true);
            } else {
                // Handle URL
                streamId = await streamManager.createStream(videoUrl, headline, description, category, false);
            }
    ...
    function setupUploadMethodSwitching() {
    ...
    console.log('Post stream page initialized successfully');
}
...
function initializeStreamsPage() {
    ...
    console.log('Initializing streams page for user:', currentUser?.email);
    ...
    streamManager.listenToStreams((streams) => {
        console.log('Real-time streams update:', streams.length, 'streams');
    ...
    console.log('Streams page initialized successfully');
}
...
function getVideoThumbnail(stream) {
    return stream.thumbnailUrl || 'images-defaultse-profile.jpg';
}
        ...
        <div class="stream-card" data-stream-id="${stream.id}">
            <div class="${videoPreviewClass}" onclick="playStream('${stream.id}')">
                ...
                <div class="video-duration">
                    <i class="fas fa-${needsConversion ? 'sync-alt' : (isPhoneVideo ? 'mobile-alt' : (isCloudinaryVideo ? 'cloud-upload-alt' : 'play-circle'))}"></i> 
                    ${needsConversion ? 'Converted Video' : (isPhoneVideo ? 'Phone Video' : (isCloudinaryVideo ? 'Uploaded Video' : 'Watch'))}
                </div>
            </div>
            <div class="stream-info">
                ...
                <div class="stream-author">
                    <img src="${stream.authorImage || 'images-defaultse-profile.jpg'}" alt="${stream.authorName}" 
                         class="author-avatar" onerror="this.src='images-defaultse-profile.jpg'"
    ...
    streams.forEach(stream => {
        ...
        if (likeBtn) {
            likeBtn.addEventListener('click', (e) => {
                e.stopPropagation(); // Prevent triggering the stream play
...
// FIXED: Toggle comments function for stream cards
...
// FIXED: Handle add comment for stream cards
...
document.addEventListener('DOMContentLoaded', async () => {
    console.log('DOM loaded, initializing stream functionality...');
    
    setupLogout();
    
    try {
        await initializeStreamPage();
        console.log('Stream page initialized successfully');
    } catch (error) {
